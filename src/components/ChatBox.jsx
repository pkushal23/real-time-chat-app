    ...
    <section className='flex flex-col items-start justify-start h-screen w-[100%] background-image'>
        <header className='w-[100%] h-[82px]m:h-fit p-4 bg-white'>
            <main className='flex items-center gap-3 '>
                <span>
                    <img src={selectedUser?.image || defaultUser} className='w-11 h-11 object-cover rounded-full' alt=""/>
                </span>
                <span>
                    <h3 className="font-semibold text-[#2A3D39] text-lg">{user2?.fullName || "Chatfrik User"}</h3>
                    <p className="font-light text-[#2A3D39] text-sm">{user2?.username || "chatfrik"}</p>
                </span>
            </main>
        </header>
                                        ...
                                        {msg?.sender === senderEmail ? (
                                            ...
                                            <div className="flex flex-col items-start w-full">
                                                <span className="flex gap-3 w-[40%] h-auto ms-10">
                                                    <img src={defaultUser} className="h-11 w-11 object-cover rounded-full" alt="" />
                                                    <div>
                                                        <div className="flex items-center bg-white justify-center p-6 rounded-lg shadow-sm">
                                                            <h4>{msg.text}</h4>
                                                        </div>
